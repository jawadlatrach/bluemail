var Bluemail = function() 
{
    ...
    {
        Bluemail.initSlimScroll('.scroller');
    };
    ...
    { 
        $('.data-list').each(function() 
        {
            ...
            Bluemail.initDataTable(table,order,page,method);
        });  
    };
    ...
    { 
        $('.summernote').each(function() 
        {
            ...
            function sendFile(file,element) 
            {
                ...
                $.ajax({
                    url: Bluemail.getBaseURL() + "/services.json",
                    data: data,
                    cache: false,
                    contentType: false,
                    processData: false,
                    ...
                    success: function(result)
                    {
                        if(result != false)
                        {
                            ...
                            if(status == 200)
                            {
                                ...
                                Bluemail.alertBox({title: result['data']['message'], type: 'success', allowOutsideClick: "true", confirmButtonClass: 'btn-primary'});
                            }
                            else
                            {
                                Bluemail.alertBox({title: result['message'], type: "error", allowOutsideClick: "true", confirmButtonClass: "btn-danger"});
                            }
    ...
    {
        var resBreakpointMd = Bluemail.getResponsiveBreakpoint('md');
        
        $('.page-sidebar').on('click', 'li > a', function (e) 
        {    
            if (Bluemail.getViewPort().width >= resBreakpointMd && $(this).parents('.page-sidebar-menu-hover-submenu').size() === 1) 
            {
                return;
            }
            ...
            {
                if (Bluemail.getViewPort().width < resBreakpointMd && $('.page-sidebar').hasClass("in")) 
                {
            ...
            if (sub.is(":visible")) 
            {
                ...
                sub.slideUp(slideSpeed, function () 
                {
                    if (autoScroll === true && $('body').hasClass('page-sidebar-closed') === false) 
                    {
                        ...
                        {
                            Bluemail.scrollTo(the, slideOffeset);
                        }
                    }
                });
            } 
    ...
    var handleFormValidations = function ()
    {
        $('form.validate :submit').on('click',function(e)
        {
            ...
            $(":input",$(this).closest('form.validate')).each(function()
            {
                ...
                {
                    Bluemail.alertBox({title:$(this).attr('data-validation-message'),type:"error",allowOutsideClick:"true",confirmButtonClass:"btn-danger"});
                    button.html(html);
     ...
     var handlesessionTimeout = function () 
     {
        if ($.sessionTimeout) 
        {
            $.sessionTimeout({
                ...
                redirUrl: Bluemail.getBaseURL() + '/authentication/logout.html',
                logoutUrl: Bluemail.getBaseURL() + '/authentication/logout.html',
                keepAliveUrl : Bluemail.getBaseURL(),
                warnAfter: 1790000,
    ...
    return {
        ...
        blockUI: function(options) 
        {
            ...
            {
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '') + '"><img src="' + Bluemail.getLayoutURL() + '/images/icons/loading-spinner-grey.gif" align=""></div>';
            } 
            ...
            {
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '') + '"><img src="' + Bluemail.getLayoutURL() + '/images/icons/loading-spinner-grey.gif" align=""><span>&nbsp;&nbsp;' + (options.message ? options.message : 'LOADING...') + '</span></div>';
            }
        ...
        {
            return Bluemail.getBaseURL() + '/layouts/main';
        },
                ...
                chart.bind("plothover", function (event, pos, item)
                {
                    ...
                    if (item)
                    {
                        if (previousPoint != item.dataIndex)
                        {
                            ...
                            $("#tooltip").remove();
                            Bluemail.showChartTooltip(item.pageX, item.pageY, item.datapoint[1] + " " + item.series.label);
                        }
                    } else
                    {
                        $("#tooltip").remove();
            ...
            for (var i = 0; i < text.length; i++) 
            {
                    ...
                    {  
                        if (0xDC00 <= cc && cc <= 0xDFFF) 
                        {
                            ...
                            outputString += ' ' + Bluemail.convertFromDecToHex(0xF0 | ((suppCP>>18) & 0x07)) + ' ' + Bluemail.convertFromDecToHex(0x80 | ((suppCP>>12) & 0x3F)) + ' ' + Bluemail.convertFromDecToHex(0x80 | ((suppCP>>6) & 0x3F)) + ' ' + Bluemail.convertFromDecToHex(0x80 | (suppCP & 0x3F));
                            highsurrogate = 0;
                            continue;
                        }
                        else 
                    ...
                    {
                        if (cc <= 0x7F) { outputString += ' ' + Bluemail.convertFromDecToHex(cc); }
                        else if (cc <= 0x7FF) { outputString += ' ' + Bluemail.convertFromDecToHex(0xC0 | ((cc>>6) & 0x1F)) + ' ' + Bluemail.convertFromDecToHex(0x80 | (cc & 0x3F)); } 
                        else if (cc <= 0xFFFF) { outputString += ' ' + Bluemail.convertFromDecToHex(0xE0 | ((cc>>12) & 0x0F)) + ' ' + Bluemail.convertFromDecToHex(0x80 | ((cc>>6) & 0x3F)) + ' ' + Bluemail.convertFromDecToHex(0x80 | (cc & 0x3F)); } 
                    }
            }
        ...
        initDataTable : function(table,orderColumns,pages,method)
        {
            ...
            var oTable = table.dataTable({
                ...
                "fnDrawCallback": function(oSettings) 
                {
                    if(callbackMethod != '')
                    {
                        Bluemail.executeFunctionByName(callbackMethod,window);
                    }
                }
            });
