...
{   
    ...
    var handleInstallingLargeServers = function () 
    {
        $('#instalation-form-large-submit').click(function(event) 
        { 
            ...
            {
                Bluemail.alertBox({title: 'No Server Selected !', type: "error", allowOutsideClick: "true", confirmButtonClass: "btn-danger"});
                return false;
            }
            
            if(IPs == null || IPs == '')
            {
                Bluemail.alertBox({title: 'Please Insert IPs ', type: "error", allowOutsideClick: "true", confirmButtonClass: "btn-danger"});
                return false;
            }
            
            if(Domains == null || Domains == '')
            {
                Bluemail.alertBox({title: 'Please Insert Domains ', type: "error", allowOutsideClick: "true", confirmButtonClass: "btn-danger"});
                return false;
            }
            
            if(subDomains == null || subDomains == '')
            {
                Bluemail.alertBox({title: 'Please Insert sub Domains ', type: "error", allowOutsideClick: "true", confirmButtonClass: "btn-danger"});
                return false;
            }
            ...
            $.ajax({
                type: 'post',
                url: Bluemail.getBaseURL() + "/servers/large/",
                data :  {
                    "server-id" : serverID,
                    "ips" : IPs,
                    "Domains" : Domains,
                ...
                success:function(result) 
                {
                    if(result !== null)
                    {
                        $("#installation-proccess").html(result['value']);
                        $("#installation-proccess").slimscroll({ scrollBy: '100px' });
                        $('#instalation-form-large-submit').click();
                    }
                },
    ...
    var handleInstallingServers = function () 
    {
        $('#instalation-form-submit').click(function(event) 
        {
            ...
            {
                Bluemail.alertBox({title: 'Please Select At Least One Instalation Proccess !', type: "error", allowOutsideClick: "true", confirmButtonClass: "btn-danger"});
                return false;
            }
            ...
            {
                Bluemail.alertBox({title: 'Please check your IP/Domains Mapping ( it seems that you\'ve missed some ) !', type: "error", allowOutsideClick: "true", confirmButtonClass: "btn-danger"});
                return false;
            }
            ...
            $.ajax({
                type: 'post',
                url: Bluemail.getBaseURL() + "/servers/install/",
                data :  {
                    "server-id" : serverId,
    ...
    var getInstallationProccess = function()
    {
        ...
        $.ajax({
                type: 'post',
                url: Bluemail.getBaseURL() + "/servers/proccess/",
                data :  {
                    "server-id" : serverId
                },
                dataType : 'JSON',
    ...
    { 
        $('#server-uninstallation').click(function(event) 
        {
            ...
            function ()
            {
                $.ajax({
                    type: 'post',
                    url: Bluemail.getBaseURL() + "/servers/uninstall/",
                    data :  {
                        "server-id" : serverId
                    },
                    dataType : 'JSON',
