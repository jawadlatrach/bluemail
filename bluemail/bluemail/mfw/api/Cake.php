<?php namespace ma\mfw\api
{
    ...
    class Cake extends Base
    {
        ...
        {
            $link = '';
            $parameters = array('api_key' => $this->getKey(),'affiliate_id' => $this->_affiliateId,'offer_id' => intval($offer['production_id']));
            $response = $this->getResponse(trim($this->_url,RDS) . RDS . 'offers.asmx' . RDS . 'GetSuppressionList', $parameters);

            if ($response != null) 
            {
                if (count($response) && key_exists('download_url', $response) && filter_var($response['download_url'], FILTER_VALIDATE_URL)) 
                {
