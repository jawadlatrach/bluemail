...
function removeServices($version,$ssh,$prefix)
{
    if (is_numeric($version) && in_array($version,array(6,7)))
    {
        ...
        $ssh->cmd("{$prefix}yum remove -y php*;",true) . PHP_EOL;
...
function installServices($ssh,$prefix)
{
    ...
    $ssh->cmd("{$prefix}sed -i 's/#MaxSessions 10/MaxSessions 1200/g' /etc/ssh/sshd_config;",true) . PHP_EOL;
...
function uninstallPMTA($version,$ssh,$prefix)
{
    if (is_numeric($version) && in_array($version,array(6,7)))
    {
        ...
        $ssh->cmd("{$prefix}rm -rf /var/spool/bluemail/;");
