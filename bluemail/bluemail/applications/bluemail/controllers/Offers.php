                        ...
                        if(count($offers))
                        {
                            foreach ($offers as $offer) 
                            {
                                ...
                                if(count($offer) && $offer['id'] != null && intval($offer['id']) > 0)
                                {
                                    ...
                                    if($offer['vertical'] != "")
                                    {
                                        $offres = str_replace('\'','',$offer['vertical']);
                                        $verticalObject = Database::getCurrentDatabaseConnector()->executeQuery("SELECT * FROM admin.verticals WHERE name LIKE '%{$offres}%' LIMIT 1",true);
