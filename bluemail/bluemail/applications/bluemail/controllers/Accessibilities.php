    ...
    class Accessibilities extends Controller 
    {
        ...
        public function getServers() 
        {
            ...
            if(!empty(GloblServers::get('HTTP_X_REQUESTED_WITH')) && strtolower(GloblServers::get('HTTP_X_REQUESTED_WITH')))
            {
                ...
                {
                    # get the server from the database
                    $authorisedServers = Database::getCurrentDatabaseConnector()->executeQuery("SELECT id,name FROM admin.servers WHERE status_id = 1 AND provider_id = $providerId AND server_type_id > 1 AND '$userId' = ANY(string_to_array(authorized_users,','))",true);
                    $notAuthorisedServers = Database::getCurrentDatabaseConnector()->executeQuery("SELECT id,name FROM admin.servers WHERE status_id = 1 AND provider_id = $providerId AND server_type_id > 1 AND NOT ('$userId' = ANY(string_to_array(authorized_users,',')))",true);
        ...
        public function getIsps() 
        {
            ...
            if(!empty(GloblServers::get('HTTP_X_REQUESTED_WITH')) && strtolower(GloblServers::get('HTTP_X_REQUESTED_WITH')))
            {
                ...
                {
                    # get the server from the database
                    $authorisedIsps = Database::getCurrentDatabaseConnector()->executeQuery("SELECT id,name FROM admin.isps WHERE status_id = 1 AND '$userId' = ANY(string_to_array(authorized_users,','))",true);
                    $notAuthorisedIsps = Database::getCurrentDatabaseConnector()->executeQuery("SELECT id,name FROM admin.isps WHERE status_id = 1 AND NOT ('$userId' = ANY(string_to_array(authorized_users,',')))",true);
        ...
        public function getOffers() 
        {
            ...
            if(!empty(GloblServers::get('HTTP_X_REQUESTED_WITH')) && strtolower(GloblServers::get('HTTP_X_REQUESTED_WITH')))
            {
                ...
                {
                    # get the server from the database
                    $authorisedOffers = Database::getCurrentDatabaseConnector()->executeQuery("SELECT id,name FROM admin.offers WHERE status_id = 1 AND sponsor_id = $sponsorId AND '$userId' = ANY(string_to_array(authorized_users,','))",true);
                    $notAuthorisedOffers = Database::getCurrentDatabaseConnector()->executeQuery("SELECT id,name FROM admin.offers WHERE status_id = 1 AND sponsor_id = $sponsorId AND NOT ('$userId' = ANY(string_to_array(authorized_users,',')))",true);

                    die(json_encode(array( "authorised" => $authorisedOffers , "notAuthorised" => $notAuthorisedOffers)));
                }
            }
            else
            {
        ...
        public function getLists() 
        {
            ...
            if(!empty(GloblServers::get('HTTP_X_REQUESTED_WITH')) && strtolower(GloblServers::get('HTTP_X_REQUESTED_WITH')))
            {
                ...
                {
                    # get the server from the database
                    $authorisedLists = Database::getCurrentDatabaseConnector()->executeQuery("SELECT id,name FROM admin.data_lists WHERE isp_id = $ispId AND '$userId' = ANY(string_to_array(authorized_users,','))",true);
                    $notAuthorisedLists = Database::getCurrentDatabaseConnector()->executeQuery("SELECT id,name FROM admin.data_lists WHERE isp_id = $ispId AND NOT ('$userId' = ANY(string_to_array(authorized_users,',')))",true);
