...
{  
    ...
    $em = key_exists('em', $_GET) ? $_GET['em'] : '';
} 
else 
{
    die('<pre>Error : No Parameters Passed !</pre>');
}
...
if (count($_POST)) 
{
    $dropId = key_exists('d', $_POST) ? intval($_POST['d']) : 0;
    $clientId = key_exists('c', $_POST) ? intval($_POST['c']) : 0;
    $listId = key_exists('l', $_POST) ? intval($_POST['l']) : 0;
    $email = key_exists('email', $_POST) ? $_POST['email'] : '';    
    $em = key_exists('em', $_POST) ? $_POST['em'] : '';
    ...
    elseif ($dropId > 0 && $listId > 0 && $clientId > 0) 
    {
        $url = '$P{API_URL}';
        $ip = getIp();
        $agent = base64_encode($_SERVER['HTTP_USER_AGENT']);
        $language = base64_encode($_SERVER['HTTP_ACCEPT_LANGUAGE']);
        $scommand = 'nohup php -r \'require_once "' . __DIR__ . '/help.php"; $result = json_decode(sendPostRequest("' . $url . '", [ "api_key" => "x0ja8s4a3duqk9e2w6vga91hrvi7t14wrdxpv754aql055tr2ee2d59b6hop", "func_name" => "procceed-tracking", "data" => [ "type" => "opt" , "drop-id" => "' . $dropId . '" , "client-id" => "' . $clientId . '" , "list-id" => "' . $listId . '" , "agent" => "' . $agent . '" , "ip" => "' . $ip . '" , "lang" => "' . $language . '" ]]),true);  print_r($result["results"]["message"]);\' >> /tmp/tracks.log 2>> /tmp/tracks.log &';
        exec($scommand);
        $message = "<span style='color:green'>Sorry to see you leaving :(</span>";
    }
    else
    {
        $message = "<span style='color:red'>Please check your information !</span>";
    }
} 
?>
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
    <head>
        <title>Unsubscribe From Our MailList</title>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
        <meta name="ops" content="unsubscribe" />
        <style type="text/css">
            ...
            input.submit		{ background: #2F2F2F; border: 1px solid #000; color: #fff; font: 15px verdana, arial; cursor: pointer; padding: 8px 14px; text-decoration: none; border-radius: 18px; -moz-border-radius: 18px; -webkit-border-radius: 18px; }
    ...
    <body style="padding: 0; margin: 0;" class="body">
        <div class="container">
            <div class="content">
                <h1 class="lang">Please specify your email address that you would like removed.</h1>
                <div class="lang"><?php echo $message;?></div>
                <form method="post" action="opt.php?d=<?php echo $dropId?>&l=<?php echo $listId?>&c=<?php echo $clientId?>&em=<?php echo $em?>" id="formSubmit" style="margin-top: 20px">
                    <table class="optoutForm" style="margin: 30px">
                        <tr>
                            <td style="white-space: nowrap; vertical-align: middle">
                                <label>
                                    <input type="hidden" name="d" value="<?php echo $dropId?>"/>
                                    <input type="hidden" name="l" value="<?php echo $listId?>"/>
                                    <input type="hidden" name="c" value="<?php echo $clientId?>"/>
                                    <input type="hidden" name="em" value="<?php echo $em?>"/>
                                    <span class="lang" style="font-size: 20px">Email</span> 
                                    <input type="text" name="email" maxlength="100" size="30" value="" style="font-size: 17px; padding: 7px; border-radius: 10px; -moz-border-radius: 10px; -webkit-border-radius: 10px;" />
                                </label> 
